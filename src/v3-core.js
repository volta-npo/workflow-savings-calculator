...
export function validateV3Definition(v3) {
    ...
    if (v3.qualityGates.length < 8)
        throw new Error('v3 quality gates must have at least 8 items');
    if (v3.schemas.length < 6)
        throw new Error('v3 schemas must have at least 6 fields');
    return true;
}
...
export function v3Warnings(v3, state) {
    ...
    if (c.criticalOpen)
        warnings.push(`${c.criticalOpen} critical launch gate(s) are not verified/certified.`);
    if (c.evidenceScore < 100)
        warnings.push(`Evidence coverage is ${c.evidenceScore}%. v3 requires evidence for every gate.`);
    for (const gate of v3.qualityGates) {
        const lower = gate.toLowerCase();
        if (lower.includes('export') && !v3.capabilities.some((cap) => cap.toLowerCase().includes('export')))
            warnings.push(`Capability missing for quality gate: ${gate}`);
        if (lower.includes('privacy') && !v3.operations.some((op) => op.toLowerCase().includes('privacy')))
            warnings.push(`Operational privacy note missing for gate: ${gate}`);
    }
...
export function importV3Bundle(config, v3, bundle) {
    if (!bundle || bundle.product?.slug !== config.slug)
        throw new Error('bundle slug does not match this product');
    validateV3Definition(v3);
    if (!bundle.state || !Array.isArray(bundle.state.checklist))
        throw new Error('bundle is missing v3 state checklist');
    if (bundle.state.checklist.length !== v3.releaseChecklist.length)
        throw new Error('bundle checklist length mismatch');
    ...
    bundle.state.checklist.forEach((row, index) => {
        if (!row || typeof row !== 'object')
            throw new Error(`invalid checklist row ${index + 1}`);
        if (!allowedStatuses.has(row.status))
            throw new Error(`invalid status in checklist row ${index + 1}`);
        if (!allowedSeverities.has(row.severity))
            throw new Error(`invalid severity in checklist row ${index + 1}`);
        if (typeof row.label !== 'string' || row.label !== v3.releaseChecklist[index])
            throw new Error(`checklist label mismatch in row ${index + 1}`);
        for (const field of ['id', 'owner', 'evidence', 'notes']) {
            if (row[field] !== undefined && typeof row[field] !== 'string')
                throw new Error(`invalid ${field} in checklist row ${index + 1}`);
        }
    });
    return bundle.state;
}
...
//# sourceMappingURL=v3-core.js.map