...
export function validateV3Definition(v3) {
  ...
  if (v3.productTier !== 'v3-production') throw new Error('product tier must be v3-production');
...
export function v3Warnings(v3, state) {
  ...
  const warnings = [];
  if (c.certification < 95) warnings.push(`Certification is ${c.certification}/100. v3 production requires 95+.`);
