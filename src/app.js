...
function saveState() {
    ...
    if (autosave)
        autosave.textContent = `Autosaved ${new Date().toLocaleTimeString()}`;
}
...
function bindDynamicEvents() {
    ...
    $$('[data-remove-evidence]').forEach((button) => button.addEventListener('click', () => { if (!confirm('Remove this evidence item?'))
        return; state.evidence = state.evidence.filter((item) => item.id !== button.dataset.removeEvidence); saveState(); render(); }));
}
...
function wireStaticEvents() {
    ...
    $('#sample').addEventListener('click', () => { if (!confirm('Load sample data? This overwrites the current local workspace.'))
        return; state = applySampleData(config); saveState(); render(); });
    $('#reset').addEventListener('click', () => { if (confirm('Reset this local workspace?')) {
