...
export function validateV1Definition(v1) {
    for (const key of ['workbench', 'table', 'metrics', 'rows', 'generators', 'validations']) {
        if (!Array.isArray(v1[key]) && key !== 'workbench' && key !== 'table')
            throw new Error(`v1.${key} must be an array`);
        if (!v1[key] || (Array.isArray(v1[key]) && v1[key].length === 0))
            throw new Error(`missing v1.${key}`);
    }
    if (v1.rows.length < 8)
        throw new Error('v1 workbench must have at least 8 domain rows');
    if (v1.metrics.length < 3)
        throw new Error('v1 workbench must have at least 3 metrics');
    return true;
}
...
export function v1ValidationWarnings(v1, state) {
    ...
    if (metrics[0].value < 80)
        warnings.push(`${v1.metrics[0]} is below v1 launch threshold (80).`);
    const blocked = rows.filter((r) => r.status === 'blocked');
    if (blocked.length)
        warnings.push(`${blocked.length} blocked row(s) must be resolved before v1 release.`);
    const missingEvidence = rows.filter((r) => ['ready', 'approved'].includes(r.status) && !String(r.evidence || '').trim());
    if (missingEvidence.length)
        warnings.push(`${missingEvidence.length} ready/approved row(s) need evidence.`);
    for (const rule of v1.validations) {
        const lower = rule.toLowerCase();
        if (lower.includes('owner') && !rows.some((r) => /owner|approval|consent/i.test(r.label) && r.status === 'approved'))
            warnings.push(`Rule needs proof: ${rule}`);
        if (lower.includes('evidence') && rows.every((r) => !String(r.evidence || '').trim()))
            warnings.push(`Rule needs evidence: ${rule}`);
    }
...
//# sourceMappingURL=v1-core.js.map