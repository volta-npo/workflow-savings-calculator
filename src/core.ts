...
export function assertValidConfig(config) {
  ...
  if (total !== 100) throw new Error(`criteria weights must sum to 100, got ${total}`);
  const ids = new Set();
  for (const item of config.criteria) {
    if (!item.id || ids.has(item.id)) throw new Error(`duplicate/missing criterion id: ${item.id}`);
    ids.add(item.id);
    if (!STATUS_MULTIPLIER.hasOwnProperty(item.defaultStatus)) throw new Error(`invalid default status for ${item.id}`);
  }
  return true;
}
...
export function calculateScore(config, state) {
  assertValidConfig(config);
  const stateById: Map<string, any> = new Map((state.criteria || []).map((item: any) => [item.id, item]));
...
export function scoreLabel(score) {
  ...
  if (score >= 55) return 'Promising, needs review';
...
export function readinessWarnings(config, state) {
  ...
  if (score < 75) warnings.push('Readiness is below 75. Keep this in draft until blockers and evidence gaps are resolved.');
  const criteria = state.criteria || [];
  const blocked = criteria.filter((item) => item.status === 'blocked');
  if (blocked.length) warnings.push(`${blocked.length} blocker(s) must be resolved before handoff.`);
  const missingEvidence = criteria.filter((item) => ['ready','approved'].includes(item.status) && !String(item.evidence || '').trim());
  if (missingEvidence.length) warnings.push(`${missingEvidence.length} ready/approved item(s) are missing evidence notes.`);
  if (!state.approvals?.mentorReview) warnings.push('Mentor review has not been recorded.');
  if (!state.approvals?.ownerApproval) warnings.push('Owner/client approval has not been recorded.');
  return warnings;
}
...
export function buildMarkdownReport(config, state) {
  ...
  else lines.push('- No readiness warnings.');
  lines.push('');
  ...
  } else lines.push('- No evidence added yet.');
  lines.push('');
