...
function install(){
  ...
  $('#domain-sample').addEventListener('click',()=>{if(confirm('Load product-specific sample data? This overwrites this product tool workspace.')){state=applyDomainSample(domain); save(); render();}});
...
function bind(){
  ...
  $$('[data-domain-row]').forEach(el=>el.addEventListener('input',e=>{const row=state.rows.find(r=>r.id===e.target.dataset.id); if(!row)return; row[e.target.dataset.domainRow]=e.target.type==='checkbox'?e.target.checked:(e.target.dataset.domainRow==='score'?Number(e.target.value):e.target.value); save(); render()}));
}
install();
